...
Scenario: Update the organization
When I add request headers:
|name           |value           |
|Content-Type 	|application/json|
Given request body: {
...
Scenario: Get Tags of an Organization
When I add request headers:
|name        |value           |
|Content-Type|application/json|
When I execute HTTP GET request for resource with URL `${trello-endpoint}/1/organizations/#{removeWrappingDoubleQuotes(${idOrganization})}/tags?key=<key>&token=<token>`
Then the response code is equal to '200'
Then the response time should be less than '2000' milliseconds

Scenario: Create a Tag in Organization
When I add request headers:
|name        |value           |
|Content-Type|application/json|
Given request body: {
...
When I execute HTTP POST request for resource with URL `${trello-endpoint}/1/organizations/#{removeWrappingDoubleQuotes(${idOrganization})}/tags`
Then the response code is equal to '400'
Then the response time should be less than '2000' milliseconds
!-- ends with 400 "invalid value for name" but documentation doesn't specify this key.
!-- with "name" parameter the response is 403 "feature not enabled"

Scenario: Update logo for an Organization
When I add request headers:
|name        |value           |
|Content-Type|application/json|
...
|string|token|<token> |                |        |
|string|key  |<key>   |                |        |
|binary|file |logo.png|application/json|logo.png|

When I execute HTTP POST request for resource with URL `${trello-endpoint}/1/organizations/#{removeWrappingDoubleQuotes(${idOrganization})}/logo`
Then the response code is equal to '200'
Then the response time should be less than '2000' milliseconds
!-- 400 "image missing", the logo is parsed to "[B@190dda57"

Scenario: Delete Logo for Organization
When I add request headers:
|name        |value           |
|Content-Type|application/json|
Given request body: {
...
When I execute HTTP DELETE request for resource with URL `${trello-endpoint}/1/organizations/#{removeWrappingDoubleQuotes(${idOrganization})}/logo`
Then the response code is equal to '200'
Then the response time should be less than '2000' milliseconds
